...
export const readApi = {
	react: (id: string, kind: string) =>
		api<{ toggled: string; counts: Array<{ kind: string; n: number }> }>(`/api/read/${id}/react`, {
			method: 'POST',
			body: { kind },
			auth: false
		}),
	...
	reactions: (id: string) =>
		api<{ counts: Array<{ kind: string; n: number }> }>(`/api/read/${id}/react`, { auth: false }),

	report: (id: string, reason: string, note = '') =>
		api<{ received: boolean }>(`/api/read/${id}/report`, {
			method: 'POST',
			body: { reason, note },
			auth: false
		})
