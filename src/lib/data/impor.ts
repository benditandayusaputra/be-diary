...
/** Impor format be-diary sendiri, Day One JSON, Journey, dan markdown biasa. */
...
async function imporZip(file: File): Promise<HasilImpor> {
	...
	const jsonFile = zip.file('be-diary-export.json') ?? zip.file(/Journal\.json$/)[0];
...
async function imporJson(teks: string): Promise<HasilImpor> {
	...
	if (data.format === 'be-diary-export' && Array.isArray(data.entries)) {
		let masuk = 0;
