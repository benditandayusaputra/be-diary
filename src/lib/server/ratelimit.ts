...
if (NONAKTIF) console.warn('[be-diary] rate limit dimatikan — jangan pernah dipakai di produksi');
...
export async function rateLimit(scope: string, id: string, l: Limit): Promise<void> {
	if (NONAKTIF) return;
	const key = `be-diary:rl:${scope}:${id}:${Math.floor(Date.now() / (l.windowSec * 1000))}`;
