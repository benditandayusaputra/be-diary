<script lang="ts">
	...
	const tautan = $derived(
		item.isAnonymous || !item.penName ? `/read/entri/${item.id}` : `/read/@${item.penName}/${item.slug}`
	);
</script>
...
>
	...
	<div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap">
		...
		<span class="t-data t-data-ink">{item.isAnonymous || !item.penName ? i18n.t.publik.anonim : item.penName}</span>
	...
	<div style="display:flex;gap:6px;flex-wrap:wrap;align-items:center">
		{#each item.tags.slice(0, 3) as t (t)}
			<a href="/read?tag={encodeURIComponent(t)}" class="tag-cip" style="text-decoration:none">{t}</a>
		{/each}
