...
export const AAD_TRANSFER = 'be-diary:transfer-blob:v1';
...
export function transferUrl(sessionId: string, secret: Uint8Array): string {
	return `be-diary://sambung?s=${sessionId}&k=${manualCode(secret)}`;
}
...
export function encodeQr(sessionId: string, secret: Uint8Array): string {
	return `be-diary://sambung?s=${sessionId}&k=${toB32(secret)}`;
}

export function decodeQr(text: string): TransferPayload | null {
	const m = /^be-diary:\/\/sambung\?s=([0-9a-f-]{36})&k=([A-Z2-7]+)$/i.exec(text.trim());
