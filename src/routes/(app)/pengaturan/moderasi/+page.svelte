...
<svelte:head><title>Moderasi · be-diary</title></svelte:head>
...
{:else}
	...
	>
					...
					<tbody>
						{#each daftar as l (l.id)}
							<tr>
								...
								>
									<a href="/read/entri/{l.entryId}" style="color:inherit">{l.title}</a>
								</td>
								<td style="padding:7px 10px;border-bottom:1px solid rgb(255 255 255 / 0.07);white-space:nowrap"
									>{l.reason}</td
								>
		...
		<span class="t-data" style="color:#8B948E">
			Hanya tulisan yang sudah diterbitkan penulisnya yang muncul di sini. Entri terenkripsi tidak
