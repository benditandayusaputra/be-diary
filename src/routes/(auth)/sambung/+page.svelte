<script lang="ts">
	...
	function terapkanQr(teks: string) {
		...
		if (!hasil) {
			toast.bahaya('Kode itu bukan kode penyambungan be-diary');
			return;
		}
...
<svelte:head><title>Sambungkan perangkat · be-diary</title></svelte:head>
			...
			<div style="display:flex;flex-direction:column;gap:var(--s-4)">
				<label class="label-medan">
					...
					<input
						type="text"
						bind:value={manual}
						placeholder="be-diary://sambung?s=…&k=…"
