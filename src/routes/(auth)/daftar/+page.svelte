<script lang="ts">
	...
	function unduh() {
		unduhTeks(
			`be-diary-pemulihan-${new Date().toISOString().slice(0, 10)}.txt`,
			`FRASA PEMULIHAN BE-DIARY\n\n${frasa.map((w, i) => `${i + 1}. ${w}`).join('\n')}\n\nSimpan berkas ini di tempat aman dan offline.\nSiapa pun yang punya 24 kata ini bisa membuka seluruh diary-mu.\n`
		);
	}
...
<svelte:head><title>Buat akun · be-diary</title></svelte:head>
