...
<svelte:head>
	<title>{data.profil.penName} · be-diary</title>
...
>
	<div style="display:flex;align-items:center;gap:var(--s-5);flex-wrap:wrap">
		...
		<div style="display:flex;flex-direction:column;gap:7px">
			<h1 class="t-judul t-xl" style="color:#E8DFC9;line-height:1">{data.profil.penName}</h1>
			...
			{#each data.profil.entri as e (e.id)}
				...
				>
					...
					<h2 class="t-judul" style="color:var(--ink);font-size:var(--text-lg);line-height:1.1">
						<a href="/read/@{data.profil.penName}/{e.slug}" style="color:inherit;text-decoration:none"
							>{e.title}</a
						>
					</h2>
					<p
