...
const SHELL = `be-diary-shell-${version}`;
const ASET = `be-diary-aset-${version}`;
const PUBLIK = `be-diary-publik-${version}`;
const LAMPIRAN = 'be-diary-lampiran';
	...
	event.waitUntil(
		(async () => {
			for (const key of await caches.keys()) {
				if (key.startsWith('be-diary-') && ![SHELL, ASET, PUBLIK, LAMPIRAN].includes(key)) {
...
sw.addEventListener('push', (event) => {
	...
	event.waitUntil(
		sw.registration.showNotification(m.judul || 'be-diary', {
			body: m.isi || 'Waktunya menulis hari ini',
			icon: '/icons/192.png',
			badge: '/icons/192.png',
			tag: 'be-diary-pengingat',
...
sw.addEventListener('notificationclick', (event) => {
	...
	event.waitUntil(
		(async () => {
			// Kalau be-diary sudah terbuka, fokuskan saja alih-alih membuka jendela baru.
