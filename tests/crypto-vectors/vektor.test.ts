...
describe('base64 dan base32', () => {
	...
	it('vektor base64 tetap', () => {
		expect(toB64(utf8('be-diary'))).toBe('YmUtZGlhcnk=');
...
describe('Argon2id', () => {
	it('vektor tetap untuk salt dan sandi tertentu', async () => {
		...
		const a = await argon2id('be-diary-test-vector', salt, TES_KDF);
		const b = await argon2id('be-diary-test-vector', salt, TES_KDF);
...
describe('HKDF-SHA256', () => {
	...
	it('info berbeda menghasilkan sub-kunci berbeda', async () => {
		...
		const a = await hkdf(ikm, 'be-diary:auth:v1');
		const b = await hkdf(ikm, 'be-diary:kek:v1');
		expect(toB64(a)).not.toBe(toB64(b));
	});

	it('panjang keluaran bisa lebih dari satu blok', async () => {
		const ikm = new Uint8Array(32).fill(9);
		const okm = await hkdf(ikm, 'be-diary:test', 100);
...
describe('BLAKE2b', () => {
	it('vektor tetap tanpa kunci', async () => {
		const h = await blake2b(utf8('be-diary'), null, 32);
		expect(h.length).toBe(32);
		const ulang = await blake2b(utf8('be-diary'), null, 32);
		expect(toB64(h)).toBe(toB64(ulang));
	});

	it('kunci mengubah keluaran', async () => {
		const key = new Uint8Array(32).fill(7);
		const a = await blake2b(utf8('be-diary'), null, 16);
		const b = await blake2b(utf8('be-diary'), key, 16);
		expect(toB64(a)).not.toBe(toB64(b));
	});
});
...
describe('protokol transfer perangkat', () => {
	...
	it('QR bukan milik Papan ditolak', () => {
		...
		expect(decodeQr('be-diary://sambung?s=bukan-uuid&k=AAAA')).toBeNull();
	});
});
